...
# 13-maha_commute_v13.sh   MAHA COMMUTE v13
#
...
#
# built            2026-09-17 22:26 UTC
# payloads, as carried, key stripped:
#   day    v13    186621 bytes  sha256 fa9edd9b2ead2e2e
#   night  v11    145105 bytes  sha256 87537aa91a27b5a3
#   all    v40    212440 bytes  sha256 fb1d3c87ddaf1404
#
...
#
#   bash 13-maha_commute_v13.sh             ask everything
#   bash 13-maha_commute_v13.sh --offline   install with what is already here
#   bash 13-maha_commute_v13.sh --online    fetch the missing dependencies first
#   bash 13-maha_commute_v13.sh --apps 13   day and all, no picker
#   bash 13-maha_commute_v13.sh --apps all  all three
#   bash 13-maha_commute_v13.sh --verify    check this file is whole, change nothing
#
...
MAHA_VERSION="v13"
MAHA_FILE="13-maha_commute_v13.sh"
...
night|night.commute|.nightcommute|v11|8087|the four night trams|night_server.py|four night trams
all|all.commute|.all.commute|v40|8084|every station around you|all_commute_server.py|stations around you"
...
<style>
  ...
  * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  /* ---- the live feed, night.commute v10 -------------------------------
     A tram that is really out there has to look different from a time in a
     timetable, or the two read as one thing and the live one is believed
     about the parts it does not know. Live rows are lit from the left by a
     bar in the line's own colour; scheduled times stay exactly as they were. */
  .livewrap { margin:2px 0 10px 42px; display:flex; flex-direction:column; gap:5px; }
  .liverow { display:flex; align-items:baseline; gap:8px; font-size:.79rem;
    color:var(--text); background:rgba(90,209,255,.07); border-left:3px solid var(--lc,#5ad1ff);
    border-radius:0 9px 9px 0; padding:6px 10px; }
  .liverow .lvcar { font-weight:800; font-variant-numeric:tabular-nums; color:var(--lc,#5ad1ff); }
  .liverow .lvwhere { color:var(--muted); flex:1; min-width:0; }
  .liverow .lveta { font-weight:800; font-variant-numeric:tabular-nums; white-space:nowrap; }
  .liverow.est .lveta { color:var(--muted); font-weight:700; }
  .livenone { font-size:.75rem; color:var(--muted); margin:2px 0 10px 42px; }

  /* The live time inside a scheduled row. day.commute's colours exactly:
     yellow is a time that came off the feed, red is how far off its timetable
     the tram is running either way, grey is dead on time. Two apps in one
     family that disagree about what yellow means are two apps to learn. */
  .legtime .le-t  { color:#e3b341; font-weight:800; }
  .legtime .le-d  { color:#f85149; font-weight:700; font-size:.76rem; }
  .legtime .le-ok { color:#8b949e; font-weight:700; font-size:.76rem; }
  .wifi { vertical-align:-1px; margin-left:5px; }
  .legtime.haslive { border-color:rgba(227,179,65,.55); }
  /* The next departure is filled solid cyan, and #e3b341 on that cannot be
     read. Same meanings, darkened until they can be read on that ground. */
  .legtime.next .le-t  { color:#5a3d00; }
  .legtime.next .le-d  { color:#7a1c16; }
  .legtime.next .le-ok { color:#2c3a42; }

  /* The pulse says the page is receiving, not merely holding a number it was
     given once. It stops the moment the feed goes stale, which is the only
     honest thing a live dot can do when nothing is arriving. */
  .lvdot { width:7px; height:7px; border-radius:50%; background:var(--lc,#5ad1ff);
    flex:none; align-self:center; animation:lvpulse 1.6s ease-in-out infinite; }
  @keyframes lvpulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.35;transform:scale(.72)} }
  .lvdot.flat { animation:none; opacity:.4; }

  /* the status strip under the map */
  .livebar { display:flex; align-items:center; gap:8px; font-size:.76rem; color:var(--muted);
    background:var(--card); border:1px solid var(--border); border-radius:11px;
    padding:8px 11px; margin-bottom:10px; }
  .livebar b { color:var(--text); font-variant-numeric:tabular-nums; }
  .livebar.warn { border-color:rgba(255,180,84,.55); color:var(--accent); }
  .livebar.bad  { border-color:rgba(255,107,107,.55); color:#ff6b6b; }
  .livebar .lvspacer { flex:1; }
  .livebar .lvlost { color:var(--muted); }

  /* the tram itself, on the map */
  .tram { display:flex; align-items:center; justify-content:center;
    width:26px; height:26px; margin:-13px 0 0 -13px; border-radius:50%;
    background:var(--c,#5ad1ff); color:#0b0f1a; font-weight:900; font-size:.72rem;
    border:2px solid #0b0f1a; box-shadow:0 0 0 2px rgba(255,255,255,.55), 0 2px 8px rgba(0,0,0,.6);
    font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif; }
  /* Reckoned and last-seen positions are inferences. They are drawn hollow so
     a glance can tell a tram ZET is locating from one this app is guessing at. */
  .tram.guess { background:transparent; color:var(--c,#5ad1ff); border-style:dashed;
    border-color:var(--c,#5ad1ff); box-shadow:none; }

  /* ---- favourite stations, night.commute v10 ---------------------------
     The star is the accent colour and nothing else on this screen is, so a
     starred station is findable by colour before it is read. */
  .favstrip { margin:0 0 12px; }
  .favhint { font-size:.75rem; color:var(--muted); padding:2px 2px 0; }
  .favrow { display:flex; flex-wrap:wrap; gap:7px; }
  .favchip { display:inline-flex; align-items:center; gap:6px;
    background:var(--card); border:1px solid var(--border); color:var(--text);
    border-radius:11px; padding:8px 11px; font:inherit; font-size:.84rem;
    font-weight:600; cursor:pointer; color:var(--accent); }
  .favchip i { font-style:normal; font-size:.62rem; font-weight:800; letter-spacing:.8px;
    text-transform:uppercase; color:#0b0f1a; background:var(--accent);
    border-radius:6px; padding:1px 5px; }
  .favchip.isfrom { border-color:var(--cyan); }
  .favchip.isto   { border-color:var(--accent); }

  /* The star in the picker. It is a target of its own, so tapping it keeps
     the station rather than choosing it, and the two are far enough apart
     that a thumb does not have to be careful. */
  .sg-item { display:flex; align-items:center; gap:9px; }
  .sg-star { flex:0 0 auto; background:none; border:none; cursor:pointer;
    color:var(--muted); font-size:1.05rem; line-height:1; padding:6px 2px; margin:-6px 0; }
  .sg-item.fav .sg-star { color:var(--accent); }
  .sg-item.fav { background:rgba(255,180,84,.07); }
  .sg-item .sg-lines { float:none; margin-left:auto; }

  /* and on the map, where it is drawn whether its line is switched on or not */
  .favmark { margin:-12px 0 0 -9px; color:#ffb454; font-size:1.15rem; line-height:1;
    text-shadow:0 0 3px #0b0f1a, 0 0 6px #0b0f1a, 0 1px 2px #000; }
</style>
</head>
<body>
  <div class="tophud">
  ...
  <div id="tab-plan">
    <div class="night-banner">Zagreb runs <b>four night trams</b> from
      ...
      and where to change if there is no direct one. A tram that is broadcasting carries a <b>green wifi</b> mark, and the yellow time beside the scheduled one is when it will really reach you.</div>
    <div class="picker">
...
function openSetup(){ refreshDots(); renderPdfMgr(); renderViewGrid(); renderNightSched();
  fetch("version",{cache:"no-store"}).then(r=>r.json())
    .then(v=>{ document.getElementById("verLine").textContent=(v.version||"v11")+" (a) · "+(v.build||""); })
...
printf "\n  ${OK}installed  night.commute v11 (a)${OFF}\n"
...
# MAHA_COMMUTE_SENTINEL v13 f345523e3147e536
